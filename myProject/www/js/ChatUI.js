function GenesysChatUI($, ndContainer, oTransport, oTransportData){
    ...
    function addMessage(sName, sText, bAgent, bSystemMessage){
        var ndContainer = $("<table>");
        var ndRow = $("<tr>");

        var ndMessage = $("<td><p><span class='name'></span></p></td>");
        var ndImage = $("<td><img src='src/img/agent.jpg' alt='Agent'></td>");
        //ndMessage.addClass((bSystemMessage)?"system":"").addClass((bAgent)?"them":"you");
        //ndImage.addClass("repImage");
        // if(sText.search("/Current Time/i"))
        //     sText = "";
        ndMessage.append(sText);
        ndMessage.find(".name").text(sName);
        console.log(bSystemMessage);
        console.log(sText);

        if(!bSystemMessage && sText.length){
            ndImage.addClass("agent");
            ndRow.append(ndImage);
        }
  
        ndRow.append(ndMessage);
        ndContainer.append(ndRow);
        ndContainer.addClass("message");
        oElements.Transcript.append(ndContainer);
